...
const slider = (function () {
	...
	// Select index
	function setIndex() {
		document.querySelector('.slider__button_current').classList.remove('slider__button_current');
		slider.style.transform = `translate(${slideIndex * -33.33}%)`;
	}

	// Event
	// dots
	document.querySelectorAll('.slider__button').forEach((dot, index) => {
		dot.addEventListener('click', () => {
			slideIndex = index;
			setIndex();
			dot.classList.add('slider__button_current');
		});
	});
})();
slider;